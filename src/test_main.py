...
if __name__ == '__main__':
    ...
    #HEX_TESTER.play_games_async(player1=create_mcts(simulator=get_hex_approx_sim()),
    #                            player2=create_mcts(simulator=get_hex_sim()), process_count=-1)
    #OTHELLO_TESTER.play_games_async(player1=create_regular_mcts(simulator=OthelloSimulator(), rollouts=100))

    #HEX_TESTER.create_dataset("hex_train_20", rollout_count=20, override=True)
    #HEX_TESTER.create_predictor(HexStatePredictor(HEX_STATE_SIZE, HEX_ACTION_SIZE), name=HEX_PROX_MODEL, train_file_name="hex_train_20", epoch_count=500, batch_size=100000)
    ...
    #OTHELLO_TESTER.create_predictor(OthelloStatePredictor(OTHELLO_STATE_SIZE, OTHELLO_ACTION_SIZE), name=OTHELLO_PROX_MODEL, train_file_name="Othello_train_1-000", batch_size=100000, epoch_count=500)
